...
# Creacion de clase
class detectormanos():
    # inicializacion de parametros para deteccion
    def __init__(self, mode=False, maxManos = 2, model_complexity = 1, Confdeteccion = 0.5, Confsegui = 0.5):
        self.mode = mode # Crea objeto, con su propia variable
        self.maxManos = maxManos
        self.compl = model_complexity
        self.Confdeteccion = Confdeteccion
        self.Confsegui = Confsegui

    # Creacion de objetos que detectan manos y dibujan
    ...
    def encontrarmanos(self, frame, dibujar = True):
        ...
        if self.resultados.multi_hand_landmarks:
            for mano in self.resultados.multi_hand_landmarks:
                if dibujar:
                    self.dibujo.draw_landmarks(frame, mano, self.mpmanos.HAND_CONNECTIONS) # Dibuja conexiones de puntos
        return frame

    # Funcion para encontrar posición
    def encontrarposicion(self, frame, ManoNum = 0, dibujarPuntos = True, dibujarBox = True, color= []):
        ...
        if self.resultados.multi_hand_landmarks:
            ...
            for id, lm in enumerate(miMano.landmark):
                alto, ancho, c = frame.shape # Extraccion de dimensiones en fps
                cx, cy = int(lm.x * ancho), int(lm.y * alto) # Conversion de informacion por pixeles
                xlista.append(cx)
                ylista.append(cy)
                self.lista.append([id, cx, cy])
                if dibujarPuntos:
                    cv2.circle(frame, (cx,cy),3,(0,0,0), cv2.FILLED) # Dibuja circulo
     ...
     # Funcion para deteccion y dibujar dedos arriba
    ...
    # Funcion detección distancia entre dedos
...
def main():
    ...
    while True:
        ret, frame = cap.read()
        # Una vez lista la imagen se envía
